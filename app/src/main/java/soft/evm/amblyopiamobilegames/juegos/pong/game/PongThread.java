...
import static soft.evm.amblyopiamobilegames.juegos.pong.game.Pong.screenX;
import static soft.evm.amblyopiamobilegames.juegos.pong.game.Pong.screenY;
...
public class PongThread extends Thread {
    ...
    int dificultad=0;//0-normal | 1-imposible
    SharedPreferences user, prefs;
    private final int vidas = 5;
    Chronometer chronometer;



    private final int PaddleHeight = screenY/4;//=85
    private final int PaddleWidth = screenX/20;//=25
    private final int BallRadius = screenY/20;//=30
     ...
     * Physics constants
     */
    private static final int PHYS_BALL_SPEED_VALUE = screenX/75;//= 8;
    private static int PHYS_BALL_SPEED;//= 8;
    private static final int PHYS_PADDLE_SPEED = screenX/50;//= 12;
    ...
    */
    ...
    PongThread(final SurfaceHolder surfaceHolder,
        ...
        int paddleHeight = a.getInt(R.styleable.PongView_paddleHeight, PaddleHeight);
        int paddleWidth = a.getInt(R.styleable.PongView_paddleWidth, PaddleWidth);
        int ballRadius = a.getInt(R.styleable.PongView_ballRadius, BallRadius);

        a.recycle();

        Paint humanPlayerPaint = new Paint();
        humanPlayerPaint.setAntiAlias(true);
        humanPlayerPaint.setColor(Colores.getOjo_sano());
        ...
        mCanvasBoundsPaint.setColor(Colores.getOjo_sano());
    ...
    public void run() {
        ...
        while (mRun) {
            if(!GameActivity.gameOver) {
                ...
                user = mContext.getSharedPreferences("Usuario", Context.MODE_PRIVATE);
                Insertar.correo=user.getString("correo", "");
                Insertar.nombre=user.getString("nombre", "");
                Insertar.nombre_juego="PONG";
                Insertar.score1=mHumanPlayer.score;
                Insertar.score2=dificultad;
        ...
        synchronized (mSurfaceHolder) {
            ...
            switch (mState) {
                ...
                case STATE_LOSE:
                    ...
                    mComputerPlayer.score++;
                    if(mComputerPlayer.score<vidas)
                        setupNewRound();
                    else
    ...
    private void updateDisplay(Canvas canvas) {
        ...
        setScoreText(mHumanPlayer.score + "    " + mComputerPlayer.score + "/" + vidas);
    ...
    private void handleHit(Player player) {
        if (player.collision > 0) {
           player.paint.setShadowLayer(10, 0, 0, Colores.getOjo_sano());
        } else {
    ...
    private void setupNewRound() {
        PHYS_BALL_SPEED = PHYS_BALL_SPEED_VALUE;
    ...
    private void handleCollision(Player player, Ball ball) {
        ...
        if(Math.abs(ball.dx)<Math.abs(ball.dy))
            ball.dx=(ball.dx/Math.abs(ball.dx))*Math.abs(ball.dy);
